...
export default function FilterBar() {
  ...
  const fetchTenders = async () => {
    try {
      ...
      console.log("fetchTenders -> data:", data);

      // This assumes `data` is an array
      if (Array.isArray(data)) {
        setTenders(data);
        setFilteredTenders(data);
      } else if (data.tenders && Array.isArray(data.tenders)) {
        setTenders(data.tenders);
        setFilteredTenders(data.tenders);
      } else {
        console.error("Unexpected tender data format:", data);
      }
    } catch (error) {
  ...
  const handleRefresh = async () => {
    try {
      ...
      if (!Array.isArray(result.tenders)) {
        console.error("Invalid tenders format:", result);
        return;
      }

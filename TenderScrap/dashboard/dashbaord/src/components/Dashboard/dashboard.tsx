...
const pieData = [
  { name: 'Youtube', value: 16.85, color: '#ff4d4d' },
  { name: 'Facebook', value: 45.36, color: '#4CAF50' },
...
const Card = ({
  ...
  <div className={`bg-white rounded-xl shadow-md p-4 border-t-4 border-${color}-500`}>
    <h2 className="text-2xl font-bold">{title}</h2>
    <p className="text-sm text-gray-500">{subtitle}</p>
    <div className={`mt-2 text-xs bg-${color}-500 text-white rounded p-1 text-center`}>
      {footer}
    </div>
  </div>
);
